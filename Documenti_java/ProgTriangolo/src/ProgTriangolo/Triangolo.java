package ProgTriangolo;

public class Triangolo {
    private float l1;
    private float l2;
    private float l3;

    Triangolo (float l1, float l2, float l3){
        this.l1 = l1;
        this.l2 = l2;
        this.l3 = l3;
    }

    public float getL1(float l1){
        return l1;
    }

    public void setL1(float l1){
        this.l1 = l1;
    }

    public float getL2(float l2){
        return l2;
    }

    public void setL2(float l2){
        this.l2 = l2;
    }

    public float getL3(float l3){
        return l3;
    }

    public void setL3(float l3){
        this.l3 = l3;
    }

    public float perimetro(){
        return l1 + l2 + l3;
    }

    public float area(){
        float s = (l1 + l2 + l3)/2;

        return (float) Math.sqrt(s*(s-l1)*(s-l2)*(s-l3));
    }

    public String tipo(){
        if(l1 == l2 && l1 == l3 && l2 == l3)
            return "Il triangolo e' equilatero";
        else if(l1 != l2 && l1 != l3 && l2 != l3)
            return "Il triangolo e' scaleno";
        else return "Il triangolo e' isoscele";
    }
}
